...
func TestPodWatchCacheUpdatesAfterPodReplacement(t *testing.T) {
	...
	assertSuggestionTexts(t, c.getPodSuggestions(ctx, namespace), []string{"web-old"})
...
func TestPodWatchCacheKeepsNamespacesIsolated(t *testing.T) {
	...
	assertSuggestionTexts(t, c.getPodSuggestions(ctx, "apps"), []string{"web-apps"})
...
func TestPodWatchCacheUpdatesContainerAndPortSuggestions(t *testing.T) {
	...
	assertSuggestionTexts(t, c.getContainerName(ctx, namespace, "web"), []string{"old"})
	assertSuggestionTexts(t, c.getPortsFromPodName(ctx, namespace, "web"), []string{"8080:8080"})
