...
func TestIsStreamingCommand(t *testing.T) {
	tests := []struct {
		...
		{input: "get pods --watch=0", want: false},
		{input: "logs pod/api -f", want: true},
		{input: "logs -f pod/api", want: true},
		{input: "logs pod/api -f=true", want: true},
		{input: "logs pod/api -f=false", want: false},
		{input: "logs pod/api --follow", want: true},
		{input: "logs --follow pod/api", want: true},
		{input: "logs pod/api --follow=true", want: true},
		{input: "logs pod/api --follow=1", want: true},
		{input: "logs pod/api --follow=t", want: true},
		{input: "logs pod/api --follow=false", want: false},
		{input: "logs pod/api --follow=0", want: false},
		{input: "logs pod/api --follow=f", want: false},
		{input: "logs pod/api --follow=false -f", want: true},
		{input: "logs pod/api -f --follow=false", want: false},
		{input: "--namespace apps logs pod/api --follow", want: true},
		{input: "--namespace=apps logs pod/api --follow", want: true},
		{input: "-n apps logs pod/api -f", want: true},
		{input: "-n=apps logs pod/api -f", want: true},
		{input: "-napps logs pod/api -f", want: true},
		{input: "--context prod --namespace=apps logs pod/api --follow=true", want: true},
		{input: "--all-namespaces logs pod/api --follow", want: true},
		{input: "--request-timeout 5s logs pod/api -f", want: true},
		{input: "--kubeconfig /tmp/kubeconfig logs pod/api --follow", want: true},
		{input: "--namespace apps logs pod/api --follow=false", want: false},
	}
